...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(()=>document.querySelector('#selected-id').textContent==='WF-011'&&document.querySelector('#evidence-content h4')?.textContent==='뒷불감시 인력 배치'&&!document.querySelector('#selected-workflow').hidden);
    ...
    await page.waitForFunction(()=>document.querySelector('#selected-id').textContent==='WF-007'&&!document.querySelector('#selected-workflow').hidden);
